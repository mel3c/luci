...
o = s:option(Flag, "remote_endpoint", translate("Remote Endpoint"), translate("Connect to remote endpoint"))
o.rmempty = false
...
if nixio.fs.access("/usr/bin/dockerd") then
	...
	o = s:option(Value, "bip", translate("Default bridge"), translate("Configure the default bridge network"))
	...
	o = s:option(ListValue, "log_level", translate("Log Level"), translate("Set the logging level"))
	...
	o = s:option(
		...
		translate(
			"Specifies where the Docker daemon will listen for client connections (default: unix:///var/run/docker.sock)"
		)
	)
...
if nixio.fs.access("/usr/bin/dockerd") and not m.uci:get_bool("dockerd", "dockerman", "remote_endpoint") then
	...
	o = s:option(
		...
		translate(
			"Which Port(s) can be accessed, it's not restricted by the Allowed Access interfaces configuration. Use this configuration with caution!"
		)
	)
				...
				for _, port in ipairs(v.Ports) do
					if port.PublicPort and port.IP and not string.find(port.IP, ":") then
						o:value(
							...
							v.Names[1]:sub(2) .. " | " .. port.PublicPort .. " | " .. port.Type
						)
					end
				end
			end
